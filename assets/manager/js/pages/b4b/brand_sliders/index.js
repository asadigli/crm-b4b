...
$(function(){
  const trComponent = (d,i) => {
    return `<tr data-id="${d.id}">
              ...
              <td>
              <span class="order-badge badge badge-${d.is_clickable ? "success" : "danger"}">${d.is_clickable ? words["Active"] : words["Deactive"]}</span>
              <input type="hidden" name="is_clickable" value="${d.is_clickable}">
              </td>
              <td>
  ...
  $(document).on("click",`[data-role="edit"]`,function(){
    ...
    let data = {
      ...
      is_clickable: parent.find(`[name="is_clickable"]`).val(),
    }
    ...
    if(data.is_clickable === "false"){
      $(`${edit_modal} [name="is_clickable"]`).prop("checked", false).change();
    }else{
      $(`${edit_modal} [name="is_clickable"]`).prop("checked", true).change();
    }
  ...
  $(document).on("click",`[data-role="save"]`,function(e){
    let data = {
      ...
      is_clickable: $(`${edit_modal} [name="is_clickable"]`).prop("checked") ? "1" : "0",
    };

    console.log(data.is_clickable);
