...
class Api {
  ...
  private static function callInternal($method = "GET",$path = null,$data = [],$url = NULL,$CI = null,$base_api_url = null, $usecret = null,$ukey = null){
    ...
    $headers = array_merge(
      [
        ...
        "ukey:" . $ukey,
        "lang:" . $lang,
        "b4buserid:" . $CI->session->userdata("id"),
        "remoteb4buserid:" . Auth::remote_id(),
        "b4busercurrency:" . Auth::currentAccountCurrency(),
        "useraddress:" . user_ip(),
  ...
  public static function callExt($method = "GET",$path = null,$data = [],$url = NULL){
    $CI = get_instance();
    $base_api_url = $CI->config->item("api_ext_path");
    $usecret = $CI->config->item("api_ext_usecret");
    $ukey = $CI->config->item("api_ext_ukey");

    return static::callInternal($method,$path,$data,$url,$CI,$base_api_url,$usecret,$ukey);
  }


  public static function callCs($url = null,$method = "GET",$params = []){
    $CI = get_instance();
    $curl = curl_init();
    $url = $CI->config->item("data_cisct_url") . $url;
    ...
    curl_setopt_array($curl, array(
      ...
      CURLOPT_CUSTOMREQUEST => $method,
      CURLOPT_HTTPHEADER =>
          $CI->config->item("data_cisct_credentials") + ['Lang: '.$lang],
    ));
