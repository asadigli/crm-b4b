...
//                                                echo 'Default', PHP_EOL;
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                    ...
                    if ((!$this->readDataOnly) || (!empty($this->loadSheetsOnly))) {
                        // active sheet index
                        // $activeTab = $xmlWorkbook->bookViews->workbookView ? intval($xmlWorkbook->bookViews->workbookView["activeTab"]) : null; // refers to old sheet index
                        $activeTab = intval($xmlWorkbook->bookViews->workbookView["activeTab"]); // refers to old sheet index

                        // keep active sheet index if sheet is still loaded, else first sheet is set as the active
                        if ($activeTab && isset($mapSheetId[$activeTab]) && $mapSheetId[$activeTab] !== null) {
