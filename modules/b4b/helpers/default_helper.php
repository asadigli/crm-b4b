...
if (!function_exists("assets")) {
  function assets($path,$global = false,$cache = false) {
    if(!$path) return "";
    $CI = get_instance();
    if (!$global) {
    ...
    $version = NULL;
    if (isset(pathinfo($path)["extension"]) && in_array(pathinfo($path)["extension"],["js","css"])) {
      $version = $cache ? ($CI->config->item("asset_version") ?: md5(date("m"))) : uniqid();
    }
...
if (!function_exists("json_response")) {
  function json_response($data,$no_auth = null,$header_key = null,$header_key_name = null){
    header('Content-Type: application/json');
    if(!$data) {
      echo json_encode(rest_response(409,"No body")); die;
    }
    ...
    $headerkey = isset($_SERVER["HTTP_HEADERKEY"]) ? $_SERVER["HTTP_HEADERKEY"] : NULL;
    $ajax_header_key = $CI->config->item("ajax-header-key");
    $pass_header = $headerkey && in_array($headerkey,$ajax_header_key);
