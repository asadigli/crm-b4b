<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="">
    <link rel="icon" href="<?= assets("image/favicon/logo-for-favicon.png", true); ?>">

    <title><?= $this->page_title ?: "Manager" ?> - <?= $this->config->item("company_name") ?></title>

     <!-- Vendors Style-->
    <link rel="stylesheet" href="<?= assets("css/libs/bootstrap.css", true) ?>">
    <link rel="stylesheet" href="<?= assets("css/libs/select2.min.css", true) ?>">

    <!-- Font -->
    <link rel="stylesheet" href="<?= assets("css/libs/fontawesome/all.min.css", true, true) ?>">
    <link rel="stylesheet" href="<?= assets("fonts/Roboto/stylesheet.css", true, true) ?>">

    <!-- Style-->
    <link rel="stylesheet" href="<?= assets("css/theme-primary.css", true, true) ?>">
    <link rel="stylesheet" href="<?= assets("css/style.css", true) ?>">
    <link rel="stylesheet" href="<?= assets("css/skin-color.css", true, true) ?>">

    <link rel="stylesheet" href="<?= assets("css/master.css") ?>">
    <link rel="stylesheet" href="<?= assets("css/master.css", true) ?>">
</head>

<body class="hold-transition theme-primary bg-img" style="background-image: url(<?= assets("image/cover/login-bg.jpg", true) ?>)">
