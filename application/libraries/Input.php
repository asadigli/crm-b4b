...
class Input {
  ...
  private function getVal($value = null,$config = []) {
    if (!is_array($value) && !is_object($value)) {
      ...
      if(isset($config["symbols"])) {
        if (!$config["symbols"] || $this->cleanSymbols) {
          $value = $this->cleanSymbols($value);
        } else if(is_array($config["symbols"]) || is_array($this->cleanSymbols)) {
          $value = str_replace(($config["symbols"] ?: $this->cleanSymbols), "",$value);
        }
      }
      ...
      if (isset($config["string"]) && !$config["string"]) {
        $value = is_string($value) ? $value : NULL;
      }
    }
    return !is_array($value) && !is_object($value) ? (is_string($value) ? trim($value) : $value) : $value;
  }
  ...
  public function delete($param,$config = []){
    ...
    return $this->getVal($value,$config);
  }
